...
class TestEvaluator:
    ...
    def test_evaluate_incorrect_output(self):
        ...
        result = asyncio.run(
            ev.evaluate(goal="Add 2+2", plan="Use +", output="The sum is 5")  # Wrong
        )
...
class TestStrategySelector:
    def test_direct_strategy_trivial(self):
        ...
        strategy, _reason = selector.select("What is Python?")

        assert strategy == StrategyType.DIRECT

    def test_planner_strategy_complex(self):
        selector = StrategySelector()
        strategy, _reason = selector.select("Build a REST API with authentication")
    ...
    def test_research_strategy(self):
        ...
        strategy, _reason = selector.select("Investigate and compare ML frameworks")
