...
from rann_agent.core.events import Event, EventEmitter, EventStatus, EventType
...
class TestEventEmitter:
    ...
    def test_get_events_filter(self):
        ...
        emitter.create_event(
            EventType.TOOL_COMPLETED, component="tools", status=EventStatus.SUCCESS
        )
...
class TestBudgetEngine:
    ...
    def test_can_make_model_call(self):
        ...
        allowed, _reason = engine.can_make_model_call(estimated_tokens=100)
        assert not allowed
...
class TestVerificationChecks:
    def test_file_exists_check(self):
        ...
        passed, evidence, _error = asyncio.run(run())
        assert passed
    ...
    def test_python_syntax_check(self):
        ...
        passed, _evidence, _error = asyncio.run(run())
        assert passed
