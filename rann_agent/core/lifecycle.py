...
from rann_agent.core.exceptions import BudgetExceededError, StateMachineError
...
class AgentLifecycle:
    ...
    async def run(self):
        ...
        try:
            ...
            self.events.emit(
                self.events.create_event(
                    EventType.ERROR_OCCURRED, status=EventStatus.FAILURE, error=str(e)
                )
            )
            raise
        ...
        try:
            ...
            self.events.emit(
                self.events.create_event(
                    ...
                    from_state=(
                        self.state_machine._history[-2].to_state.value
                        if len(self.state_machine._history) > 1
                        else None
                    ),
    ...
    def check_budget(self) -> None:
        ...
        if self.budget_engine.tracker.is_exhausted():
            ...
            raise BudgetExceededError("Budget exhausted", details=status)
    ...
    def checkpoint(self, data: dict[str, Any], reason: str = "manual") -> None:
        ...
        self.events.emit(
            self.events.create_event(
                EventType.CHECKPOINT_CREATED, status=EventStatus.SUCCESS, reason=reason
            )
        )
