...
class VerificationEngine:
    ...
    def add_assertion(
        ...
        self.add_check(
            VerificationCheck(
                name=name, description=description, verify=verify_fn, required=required
            )
        )

    async def verify(
        self, task: str, output: Any, context: dict[str, Any] | None = None
    ) -> VerificationResult:
        """
        ...
        for check in self._checks:
            try:
                ...
                result.errors.append(f"{check.name}: {e!s}")
                if check.required:
                    result.status = VerificationStatus.FAILED
...
class VerificationChecks:
    ...
    def file_exists(path: str) -> VerificationCheck:
        ...
        return VerificationCheck(
            name=f"file_exists:{path}", description=f"File {path} exists", verify=verify
        )
    ...
    @staticmethod
    def command_succeeds(command: str, cwd: str | None = None) -> VerificationCheck:
    ...
    def git_no_uncommitted_changes(repo_path: str) -> VerificationCheck:
        ...
        return VerificationCheck(
            name="git_clean", description="No uncommitted changes", verify=verify
        )
