...
class UserModel:
    ...
    async def update_from_interaction(
        self, message: str, response: str, metadata: dict[str, Any] | None = None
    ):
        """Update user model from an interaction."""
        self.profile["interaction_count"] += 1
        self.profile["last_seen"] = datetime.now(UTC).isoformat()
    ...
    async def _update_habits(self, message: str, metadata: dict | None = None):
    ...
    async def _analyze_communication_style(self, message: str):
        ...
        if "avg_message_length" not in style:
            ...
            style["avg_message_length"] = (
                style["avg_message_length"] * (count - 1) + length
            ) / count
