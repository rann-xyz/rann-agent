...
class ProceduralMemory:
    ...
    def search(self, query: str, category: str | None = None) -> list[Skill]:
        ...
        return sorted(
            results,
            key=lambda s: s.success_count / max(1, s.success_count + s.failure_count),
            reverse=True,
        )
    ...
    def record_execution(self, skill_id: str, success: bool, execution_time_ms: float) -> None:
        ...
        skill.avg_execution_time_ms = (
            skill.avg_execution_time_ms * (n - 1) + execution_time_ms
        ) / n

    def get_best_skill(self, task: str, category: str | None = None) -> Skill | None:
