...
class CodeCompletion:
    ...
    def _suggest_control_flow(self, context: dict[str, Any], language: str) -> list[dict[str, Any]]:
        ...
        if keyword == "if":
            suggestions.append(
                {"code": f"\n{indent}pass", "description": "If body", "confidence": 0.8}
            )
    ...
    def _suggest_general(self, code: str, language: str) -> list[dict[str, Any]]:
        ...
        if "async" in code:
            suggestions.append(
                {"code": "await ", "description": "Await async call", "confidence": 0.6}
            )
