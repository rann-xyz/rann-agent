...
class CodebaseContext:
    ...
    async def index_codebase(self, extensions: list[str] | None = None) -> dict[str, Any]:
        ...
        stats = {"total_files": 0, "total_lines": 0, "languages": {}, "modules": []}
    ...
    async def get_file_context(self, file_path: str) -> dict[str, Any] | None:
    ...
    async def get_related_files(self, file_path: str) -> list[str]:
        ...
        if file_path in self.symbol_index:
            ...
            for imp in imports:
                for indexed_file in self.symbol_index:
                    if (
                        imp in indexed_file
                        or indexed_file.replace("/", ".").replace(".py", "") in imp
                    ):
