...
class CodeIntelligenceTool(Tool):
    ...
    parameters = {
        "action": {
            "type": "string",
            "required": True,
        },  # analyze | generate_tests | suggest_refactor | generate_docs
    ...
    async def execute(
        self, action: str, path: str, options: dict | None = None, **kwargs
    ) -> dict[str, Any]:
    ...
    async def _generate_documentation(self, file_path: Path) -> dict[str, Any]:
        ...
        return ToolResult(
            tool=self.name, success=True, output="Documentation generation coming soon!"
        ).to_dict()
