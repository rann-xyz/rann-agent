...
class TestRunnerTool(Tool):
    ...
    async def execute(
        self, framework: str, path: str = ".", options: list | None = None, **kwargs
    ) -> dict[str, Any]:
        """Run tests"""
...
class LinterTool(Tool):
    ...
    parameters = {
        "tool": {
            "type": "string",
            "required": True,
        },  # ruff | black | eslint | prettier | gofmt
    ...
    async def execute(
        ...
        try:
            if tool == "ruff":
                ...
                return ToolResult(
                    tool=self.name, success=False, error=f"Unknown linter: {tool}"
                ).to_dict()
            ...
            return ToolResult(
                ...
                success=result.returncode == 0,
                output=output,
                metadata={"linter": tool, "fixed": fix, "exit_code": result.returncode},
            ).to_dict()
