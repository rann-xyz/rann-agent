...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not path.startswith("/api/"):
            ...
            if file_path.is_file():
                ...
                self.end_headers()
                self.wfile.write(
                    b"<h1>RANN Web API</h1><p>API endpoints: /api/chat, /api/status, /api/providers</p>"
                )
                return
            ...
            self.send_json(
                {
                    ...
                    "providers": {
                        name: {"has_key": name in keys, "free": prov["free"]}
            ...
            self.send_json(
                {
                    ...
                    "has_key": has_key,
                    "model": config.get(
                        f"{provider}_model", PROVIDERS.get(provider, {}).get("default_model", "")
                    ),
                    "base_url": config.get(
                        f"{provider}_url", PROVIDERS.get(provider, {}).get("base_url", "")
                    ),
                }
            )
    ...
    def chat_openai(self, api_key, base_url, model, message, history):
        ...
        messages = [
            {
                "role": "system",
                "content": "You are RANN Agent. Be concise and helpful. Format code with triple backticks.",
            }
        ]
        for msg in history:
    ...
    def chat_anthropic(self, api_key, base_url, model, message, history):
        ...
        payload = json.dumps({"model": model, "messages": messages, "max_tokens": 2000}).encode(
            "utf-8"
        )
