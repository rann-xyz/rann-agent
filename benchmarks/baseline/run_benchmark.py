...
class BenchmarkMetrics:
    ...
    def finalize(self):
        ...
        # Tool success rates
        for data in self.metrics["tool_success"].values():
...
async def benchmark_simple_task():
    ...
    try:
        ...
        with patch("rann_agent.core.agent.LLMProvider", return_value=mock_llm):
            ...
            mock_llm.complete_with_retry = AsyncMock(
                return_value={
...
async def benchmark_tool_execution():
    ...
    from rann_agent.tools.files import FileReadTool
...
async def run_baseline_benchmark():
    ...
    benchmark.record_task(
        task_result["success"], task_result["first_attempt"], task_result["latency"]
    )
    ...
    with open(output_dir / "baseline_summary.txt", "w") as f:
        ...
        for tool, data in summary.get("tool_success", {}).items():
            f.write(
                f"  {tool}: {data.get('success_rate', 0):.1%} ({data.get('success', 0)}/{data.get('success', 0) + data.get('failure', 0)})\n"
            )
